...
namespace VoIP.WinFormsUserInterface
{
    ...
    {
        private const string FORM_TITLE = "VoIP";
        ...
        private static int _logOut;

        TIPClient.TCPClient tcpclient;
        TIPPacket.Packet message;
        private string myEmail;
        public MainForm(TIPClient.TCPClient client, string Email)
        {
            InitializeComponent();
        ...
        private void mainPanel_logOut(string hostname, int port)
        {
            listener.Dispose();
        ...
        private void SetFriendsPanel(TIPClient.TCPClient client, string email)
        {
        ...
        {
            isBusy = false;
            MainPanel callPanel = new MainPanel(tcpclient, myEmail);
            callPanel.BeginCall += mainPanel_BeginCall;
            callPanel.LogOut += mainPanel_logOut;
            SetNonePanel(callPanel);
        }
        ...
        private void SetNonePanel(PanelBase panel)
        {
            foreach (IDisposable c in panelLayout.Controls)
                c.Dispose();
        ...
        private void QuitButton_Click(object sender, EventArgs e)
        {
            ...
            if (waitForAnswer(tcpclient.Send(TIPPacket.Command.EndConnection, "", tcpclient.getStream())))
            {
                if (message.Command == TIPPacket.Command.EndConnectionAck)
                {
                    Application.Exit();
